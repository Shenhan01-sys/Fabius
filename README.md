...
| Penilaian hasil (`ledger.py`) | ✅ ada dan sengaja belum menghasilkan angka (25 Sep): 20 rekaman dibaca → **2 peristiwa posisi unik** (3 salinan identik digabung; `decisionHash` beda karena memuat waktu generasi, peristiwanya sama), dua-duanya `BELUM JATUH TEMPO` (tempo 25 Sep 17:00Z & 18:00Z). Harga masuk dibaca dari **rekaman**, bukan dihitung ulang; "belum waktunya" punya status sendiri dan tidak ikut agregat; stop+target di bar yang sama = `AMBIGU` |

...
| Anchor keputusan SUNGGUHAN ke chain | ✅ `tools/anchor.py` (25 Sep): membaca `decisions/direction-*.jsonl` dan mengirim 3 hash-nya apa adanya, lalu **membaca ulang `getAnchor(id)` dan membandingkan word per word**. Dua batch = **13 anchor** (`Enter=2` / **`Abstain=11`**), gas 230.899–250.807/anchor. Bukan hash uji: yang masuk chain termasuk penolakan, dengan alasannya di-`gatesHash`. `--verify` mengulang pemeriksaan itu **tanpa kunci dan tanpa gas** (id dihitung ulang dari file, lalu dibaca dari chain): **11/11 cocok pada 6 field** termasuk `asset` — field yang dulu terbaca sampah dan membuat kalimat "chain == lokal" lebih luas dari yang dibandingkan (`vault/07`) |
